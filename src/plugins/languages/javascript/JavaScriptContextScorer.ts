...
export class JavaScriptContextScorer extends ContextScorer {
  constructor() {
    super({
      fileTypeWeights: {
        'source': 1.0,
  ...
  private async analyzeFileMetadata(filePath: string): Promise<FileMetadata> {
    ...
    const language = this.determineLanguage(ext);
  ...
  private determineFileType(filePath: string): string {
    ...
    if (basename.includes('.config.') || basename.includes('.conf.')) return 'config';
    if (basename.includes('.stories.')) return 'story';
  ...
  private determineLanguage(ext: string): string {
  ...
  private determineImportance(filePath: string, type: string): 'critical' | 'high' | 'medium' | 'low' {
    ...
    if (basename === 'index.js' || basename === 'index.ts') return 'critical';
    if (basename === 'main.js' || basename === 'main.ts') return 'critical';
    if (basename === 'app.js' || basename === 'app.ts') return 'high';
  ...
  private determineCategory(filePath: string): string {
    ...
    if (dir.includes('components') || dir.includes('component')) return 'component';
    if (dir.includes('utils') || dir.includes('util')) return 'utility';
    if (dir.includes('services') || dir.includes('service')) return 'service';
    if (dir.includes('store') || dir.includes('redux')) return 'state';
    if (dir.includes('api')) return 'api';
    if (dir.includes('config')) return 'configuration';
    if (dir.includes('test') || dir.includes('__tests__')) return 'test';
  ...
  private calculateTypeScore(metadata: FileMetadata, reasons: ScoringReason[]): number {
    ...
    reasons.push({
      factor: 'file_type',
      weight: baseScore,
      description: `File type: ${metadata.type}`
    });
  ...
  private calculateTaskRelevanceScore(filePath: string, task: string, reasons: ScoringReason[]): number {
    ...
    if (taskLower.includes('component') && basename.includes('component')) {
      relevance += 0.4;
      reasons.push({
        factor: 'component_relevance',
        weight: 0.4,
        description: 'Component file for component task'
      });
    }
  ...
  private calculateFrameworkScore(filePath: string, projectContext: any, reasons: ScoringReason[]): number {
    ...
    if (framework === 'react' && (filePathLower.includes('jsx') || filePathLower.includes('tsx'))) {
      score += 0.3;
      reasons.push({
    ...
    if (framework === 'vue' && filePathLower.includes('.vue')) {
      score += 0.3;
      reasons.push({
        factor: 'framework_match',
        weight: 0.3,
        description: 'Vue component file'
      });
    }
    
    if (framework === 'angular' && (filePathLower.includes('.service.') || filePathLower.includes('.component.'))) {
      score += 0.3;
      reasons.push({
        factor: 'framework_match',
        weight: 0.3,
        description: 'Angular service/component file'
      });
    }
